...
(function($){
	 ...
	 * @type {object}
	 */
	var Yii = window.Think;

	/* 基础对象检测 */
	Yii || $.error("Yii基础配置没有正确加载！");
	...
	Yii.parse_name = function(name, type){
		if(type){
			/* 下划线转驼峰 */
	...
	Yii.U = function(url, vars, suffix){
		...
		if(0 === url.indexOf("/")){ //路由模式
			...
			path[1] || $.error("Yii.U(" + url + ")没有指定控制器");
	...
	/* 设置表单的值 */
	Yii.setValue = function(name, value){
